...
export class MovieApp {
    ...
    private async init() {
        await this.loadMovies("spiderman");
    ...
    private render() {
        if (this.allMovies.length > 0) {
            this.movieApi.showMoviesList(this.allMovies, this.movieIndex, (index) => this.onMovieClick(index));
            this.movieApi.showMovieDetails(this.allMovies[this.movieIndex]);
